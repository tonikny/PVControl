# Version 30/Nov/2020 --  1 Rele SSR + 2 Reles Mecanicos
...
ESP= 32 # 1= NodeMCU, 32= ESP32
logica= 'pos' # pos= positiva ,  neg==negativa
# Mejor opcion logica neg en NodeMCU y pos en ESP32

tipo_ssr = 'AF' # AF (angulo fase)  - SC (Semiciclos)
...
#config mqtt/red
SERVER = '192.168.0.14' # IP de la Raspberry
PORT = 1883
USER = 'rpi'
PASSWORD = 'fv'
...
if tipo_ssr == 'AF': # control por Angulo de Fase
...
def sub_cb_af(topic, msg): # SSR con Angulo de Fase
  ...
  try:
    ...
    if topic[:-1] == Nodo:
      ...
      print('salida(',x,')=',y, 'PWM - Vdac=',v_dac, '/', voltios,'V')
      print ('-' * 40)
      
      ee='22a'
      if topic[-1:] == b'1':
        dac1.write(v_dac) #salida rele1 0-3V3
        #print ('v_dac1=',v_dac)
        
      elif topic[-1:] == b'2':
        dac2.write(v_dac) #salida rele2 0-3V3
        #print ('v_dac2=',v_dac)
