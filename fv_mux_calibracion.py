...
try:
    sql = """CREATE TABLE IF NOT EXISTS `parametros1` (
              ...
              `valor` varchar(500) CHARACTER SET latin1 COLLATE latin1_spanish_ci NOT NULL,
...
VceldaRatio  = d_['r_mux'] # inicializo ratio conversion divisores tension
...
while True:
    ...
    for K in range(usar_mux):  # For para ir recorriendo cada entrada del Mux
        ...
        while True:
            try:
                ...
                print('-ERROR MEDIDA MUX-'+ str(K+1))
            ...
            DatosMux_n['C'+str(K+1)] = lectura_ADS  # Valor numerico capturado
            DatosMux_err['C'+str(K+1)] = Max-Min    # Rango error valor numerico capturado
            DatosMux_v['C'+str(K+1)] = round(lectura_ADS * 0.000125/gain_mux * d_['r_mux'][K],4) # Valor V en conector.. 4,096V/32767=0.000125    
            DatosMux_v_l['C'+str(K+1)] = round(lectura_ADS * 0.125/gain_mux ,4)   # Valor mV en ADS  

            print (Fore.RED, '=' * 80)
            print (Fore.BLUE, f'Voltaje medido con Ratio actual={DatosMux_v["C"+str(K+1)]:.4f} -- ', end='')
            print (f'mV en ADS= {DatosMux_v_l["C"+str(K+1)]:.2f} --- Ratio actual={d_["r_mux"][K]:.4f}')

            VceldaReal[K] = click.prompt(Fore.CYAN + 'Voltaje real en  '+Fore.RESET+f'celda{K+1}='+ Fore.CYAN, type=float, default=DatosMux_v["C"+str(K+1)])
            if DatosMux_v_l['C'+str(K+1)] != 0:
                VceldaRatio[K] = round(VceldaReal[K] / DatosMux_v_l['C'+str(K+1)]*1000,5)
            else:
                VceldaRatio[K] = 0
            ...
            try:
                click.confirm('Otra Medida sobre la misma celda', abort=True)
            except:
                break
    ...
    if captura_mux == 'D': #lectura modo diferencial
        ...
        for K in range(2,usar_mux+1):DatosMux['C'+str(K)] = round(DatosMux_v['C'+str(K)] - DatosMux_v['C'+str(K-1)],2)
