...
<div style="display: flex; flex-direction: row">
  ...
  <div class="relojes">
    <div id="containervbat" class="reloj" style="width: 180px; height: 180px;"></div>
    <div id="containeribat" class="reloj" style="width: 180px; height: 180px;"></div>
    <div id="containertemp" class="reloj" style="width: 180px; height: 180px;"></div>
    <div id="containervplaca" class="reloj" style="width: 180px; height: 180px;"></div>
    <div id="containerconsumo" class="reloj" style="width: 180px; height: 180px;"></div>
    <div id="containerwplaca" class="reloj" style="width: 180px; height: 180px;"></div>
    <div id="containerSOC" class="reloj" style="width: 200px; height: 180px;"></div>
  </div>
  ...
  $(function () {
    ...
    chart_vbat = new Highcharts.Chart({
		...
		yAxis: {
			...
			showLastLabel:true,
			showFirstLabel:true,
			minorTickInterval: 'auto', //null, 'auto'
			minorTickWidth: 0,
			minorTickLength: 10,
			minorTickPosition: 'inside',	
			minorTickColor: '#666',
			tickPositions: [Vbat_bajo_amarillo,Vbat_verde,Vbat_alto_amarillo,Vbat_alto_rojo],
			tickPixelInterval: null,	//null, 30
			tickWidth: 2,
			tickPosition: 'inside',
			lineWidth: 0,
			tickLength: 12,
    ...
    chart_soc = new Highcharts.Chart({
		...
		yAxis: [{
			...
			showFirstLabel:false,
			stops: [
				[0.2, 'rgba(223,83,83,0.5)'], // red'#DF5353'
				[0.4, 'rgba(221,223,13,0.5)'], // yellow'#DDDF0D'
				[0.65, 'rgba(85,191,59,0.5)'] // green'#55BF3B'
				],
			lineWidth: 0,
			tickWidth: 0,
			minorTickInterval: 0, //null,
			tickAmount: 2,
			title: {		//datos de tabs/tflot
				y: 80,//-30,
				floating: true,
				//align: 'right',
			...
			showFirstLabel:false,
			stops: [
				[0.2, 'rgba(223,83,83,0.2)'], // red'#DF5353'
				[0.4, 'rgba(221,223,13,0.2)'], // yellow'#DDDF0D'
				[0.65, 'rgba(85,191,59,0.2)'] // green'#55BF3B'
				],
			lineWidth: 0,
			tickWidth: 0,
			minorTickInterval: 0, //null,
		...
		series: [{
			...
			dataLabels: {
				...
				format: 
					...
					((Highcharts.theme && Highcharts.theme.contrastTextColor) || color_rotulos) + 
					'">{y}</span>' +
    ...
    chart_temp = new Highcharts.Chart({
		...
		yAxis: [{
			...
			tickPositions: [Temp_bat_baja,Temp_bat_normal,Temp_bat_alta],
			...
			minorTickPosition: 'inside',
			minorTickColor: 'white',
			tickPositions: [Temp_rpi_normal,Temp_rpi_alta],
			tickPixelInterval: 30,
			tickInterval: 20,
			tickWidth: 2,
			tickPosition: 'inside',
    ...
    chart_consumo = new Highcharts.Chart({
      ...
      yAxis: [
        {//Wconsum0
          ...
          minorTickPosition: 'inside',
          minorTickColor: '#666',
		  tickPositions: [Consumo_watios_amarillo,Consumo_watios_rojo],
          tickPixelInterval: 30,
          tickInterval: 1000,
          tickWidth: 2,
          tickPosition: 'inside',
        ...
        {// Aconsumo
          ...
          minorTickPosition: 'inside',
          minorTickColor: '#666',
		  tickPositions: [Consumo_amperios_amarillo,Consumo_amperios_rojo],
          tickPixelInterval: 30,
    ...
    chart_ibat = new Highcharts.Chart({
      ...
      yAxis: {
        ...
        minorTickPosition: 'inside',
        minorTickColor: '#666',
		tickPositions: [Intensidad_descarga_amarillo,0,Intensidad_carga_rojo],
      ...
      series: [{
        ...
        dataLabels: {
          ...
          formatter: function () {
            return Highcharts.numberFormat(this.y, 1) + " A Bat"
          },
        },
        dial: {
			radius: '86%',
        ...
        dataLabels: {
          ...
          formatter: function () {
            return Highcharts.numberFormat(this.y, 1) + " A Placa"
          },
        },
        dial: {
			radius: '86%',
    ...
    chart_wplaca = new Highcharts.Chart({
      ...
      yAxis: {
        ...
        minorTickPosition: 'inside',
        minorTickColor: '#666',
		tickPositions: [Watios_placa_baja_rojo,Watios_placa_verde,Watios_placa_alta_amarillo],
        tickPixelInterval: 30,
        tickInterval: 1000,
        tickWidth: 2,
        tickPosition: 'inside',
    ...
    chart_vplaca = new Highcharts.Chart({
      ...
      yAxis: {
        ...
        tickWidth: 2,
        tickPosition: 'inside',
		tickPositions: [Vplaca_baja_amarillo,Vplaca_baja_verde,Vplaca_verde,Vplaca_alta_amarillo],
        tickLength: 10,
        tickColor: 'white',
        labels: {
          step: 5,
      ...
      success: function (data) {
        try {
          ...
          chart_vbat.caption.update({
            text: Number(Vbat_min_dia.toFixed(1)) + '/' + Number(Vbat_max_dia.toFixed(1)) + ' V'
          });

          // Actualizacion reloj Ibat/Iplaca
          if (Ibat <= Intensidad_descarga_amarillo) { color = 'red'; }
          else if (Ibat < 0) { color = 'orange'; }
          else if (Ibat < Intensidad_carga_rojo) { color = '#18F905'; }
          else { color = 'red'; };

          chart_ibat.series[0].update({
            data: [Ibat],
            dataLabels: { style: { color: color }, }
          });

          if (Iplaca <= Intensidad_descarga_amarillo) { color = 'red'; }
          else if (Iplaca < 0) { color = 'orange'; }
          else if (Iplaca < Intensidad_carga_rojo) { color = '#18F905'; }
          else { color = 'red'; };

          chart_ibat.series[1].update({
            data: [Iplaca],
            dataLabels: { style: { color: color }, }
          });

          // Actualizacion SOC   
          if (SOC >= 98) { color = 'green'; }
          else if (SOC > (SOC_min + 20)) { color = 'black'; }
          else if (SOC > (SOC_min + 10)) { color = 'orange'; }
          else { color = 'red'; };

			chart_soc.series[2].update({
            data: [SOC_min_dia,SOC_max_dia]
			});

          chart_soc.series[0].update({
            data: [SOC],
            dataLabels: {
              format: '<div style="text-align:center"><span style="font-size:20px;color:' +
                ((Highcharts.theme && Highcharts.theme.contrastTextColor) || color) + '">{y}</span>' +
                '<span style="font-size:15px;color:silver">%</span></div>'
            },
          });
		  
		  chart_soc.series[1].update({
            data: [SOC_max_dia]
			});

          chart_soc.yAxis[0].setTitle({
            text: 'Tabs/Tflot=' + Tabs + '/' + Tflot + 'sg - Flot_Bulk=' + Tflot_bulk // Tabs /Tflot
          });

          // Actualizacion reloj Temp
          if (Temp <= Temp_bat_baja) { color = 'red'; }
          else if (Temp < Temp_bat_normal) { color = 'orange'; }
          else if (Temp < Temp_bat_alta) { color = '#18F905'; } // Verde claro
          else { color = 'red'; };

          chart_temp.series[0].update({
            data: [Temp],
            dataLabels: { style: { color: color }, }
          });

          if ([data['TEMP']['Temp_cpu']] <= 10) { color = 'red'; }
          else if ([data['TEMP']['Temp_cpu']] < Temp_rpi_normal) { color = '#18F905'; }
          else if ([data['TEMP']['Temp_cpu']] < Temp_rpi_alta) { color = 'orange'; } // Verde claro
          else { color = 'red'; };

          chart_temp.series[1].update({
            data: [data['TEMP']['Temp_cpu']],
            dataLabels: { style: { color: color }, }
          });
          ...
          Iconsumo = Iplaca - Ibat - Ired
          if (Iconsumo <= Consumo_amperios_amarillo) { color = '#18F905'; }
          else if (Iconsumo  < Consumo_amperios_rojo) { color = 'orange'; }
          else { color = 'red'; };

          chart_consumo.series[1].update({
              data: [Iconsumo],
              dataLabels: { style: { color: color }, }
          });
