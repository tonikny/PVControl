...
def captura_ads(ads_actual, indice_ads):
    ...
    """
    # Crear instancias locales de los gestores para cada proceso
    from helpers.gestor_parametros import GestorParametros
    from helpers.gestor_bd import GestorBD
    from helpers.gestor_logs import GestorLogs

    gp_local = GestorParametros()
    logger_local = GestorLogs(f"{__name__}-{ads_actual['id']}")
...
def main():
    ...
    log_local = GestorLogs(nombre=__name__)
    log_local.info(Style.BRIGHT + Fore.YELLOW + "Arrancando" + Fore.GREEN + " fv_ads.py")
