...
nombres_datos = ['Tiempo_sg','Tiempo','Ibat', 'Vbat', 'SOC','DS','Vaux',
                 'kWhp_bat','kWhn_bat','Iplaca','Vplaca','kWh_placa','Temp','PWM']
...
def Calcular_PID (sensor,objetivo,P,I,D):
    ...
    Lista_errores_PID[0] = valor - objetivo

    #print('Lista_errores =',Lista_errores)

    # Calculo Termino Proporcional PID
    error_actual = valor - objetivo
...
try:
    ...
    sql='SELECT DS, DATE(Tiempo),kWhp_bat,kWhn_bat,kWh_placa FROM datos ORDER BY id DESC limit 1'
    cursor.execute(sql)
    var=cursor.fetchone()
    DS=float(var[0])
    HOY=str(var[1])
    if HOY == time.strftime("%Y-%m-%d"): #Comprueba que es el mismo dia
        kWhp_bat=float(var[2])
        kWhn_bat=float(var[3])
        kWh_placa=float(var[4])
    else:
         kWhp_bat=0.0
         kWhn_bat=0.0
         kWh_placa=0.0
...
while True:
    ...
    if simular == 1:
        ...
        Iplaca= 350
        Ibat = Iplaca - PWM - Consumo_lista[nbucle_m]
        
        Vbat_ant = Vbat
        Vbat = 27+3*(Ibat-5)/100
        if Vbat-Vbat_ant > 2: Vbat =Vbat+1
        if Vbat-Vbat_ant <= 2: Vbat =Vbat-1
    ...
    # Control Excedentes...calculo salida PWM
    ...
    #Vbat_final = 27+3*(Ibat-5)/100
    #Vbat_incr = Vbat_final - Vbat
    #Vbat= Vbat + Vbat_incr/20
    #print (nbucle,nbucle_m,'Consumo=',Consumo_lista[nbucle_m],'PWM=',PWM,'Ibat=',Ibat,'Vbat=',Vbat)
    ...
    with open('/run/shm/datos_fv.csv', mode='w') as f:
        ...
        datos.writerow({'Tiempo_sg': tiempo_sg,'Tiempo': tiempo,'Ibat': Ibat,'Vbat': Vbat,'SOC': SOC,'DS':DS,'Vaux':Vaux,
                        ...
                        'kWh_placa':kWh_placa,'Temp':Temp,'PWM':PWM})
 ...
 # -------------------- Bucle de condiciones de parametros FV --------------------------
    ...
    if TP[0] == "S" and Grabar == 1:
        try:
            cursor.execute("""INSERT INTO datos (Tiempo,Ibat,Vbat,SOC,DS,Vaux,kWhp_bat,kWhn_bat,Iplaca,Vplaca,kWh_placa,Temp,PWM) 
               VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
               (tiempo,Ibat,Vbat,SOC,DS,Vaux,kWhp_bat,kWhn_bat,Iplaca,Vplaca,kWh_placa,Temp,PWM))
            db.commit()
        except:
            db.rollback()
