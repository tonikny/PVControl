...
Mtemp = 60      # Numero de muestras para leer temperatura
...
def leer_aux2(x) :  # leer Aux2
    try:
        if Aux2_sensor == 'ADS':
            y = round(adc1.read_adc(3, gain=RES1_gain) * 0.000125/RES1_gain * RES1, 2)  # A3   4,096V/32767=0.000125 
        else:
            y=0
    except:
        y = x
...
Rele_Out = [[0] * 8 for i in range(40)]
Rele_Out_Ant = [[0] * 8 for i in range(40)] # Situacion actual y anterior de los reles
...
if DEBUG >= 10: print ('PWM_Max=',PWM_Max)
...
while True:
  ...
  ### ------------------ Control Excedentes...Cálculo salida PWM ----------
    ...
    #TIPO_DIVER = TP[6].upper() # AUX1 o AUX2 o VPLACA o VBAT
  ...
  ## -------- CALCULO SOC% A C20 ----------
    ...
    if (Ibat>0 and Ibat<1 and abs(Vbat-vflotacion)<0.2) : DS = DS + (AH-DS)/50
  ...
  ## ------- Salida por pantalla OLED -------
    ...
    with open('/run/shm/datos_fv.csv', mode='w') as f:
        ...
        datos.writerow({'Tiempo_sg': tiempo_sg,'Tiempo': tiempo,'Ibat': Ibat,'Vbat': Vbat,'SOC': SOC,
                        ...
                        'Iplaca':Iplaca,'Vplaca':Vplaca,'Wplaca':Wplaca,'Wh_placa':Wh_placa,'Temp':Temp,'PWM':PWM})
 ...
 # -------------------- Bucle de condiciones de parametros FV --------------------------
    ...
    for I in range(nreles):
        ...
        if Rele_Out[addr][Puerto] == 100 and Flag_Rele_Encendido == 0 and Rele_Out_Ant[addr][Puerto] < 100 :
            ...
            if DEBUG >= 1:print ('Enciendo rele ',addr,Puerto+1)
            act_rele(addr,Puerto+1,100)

        ### apagar rele
        if Rele_Out[addr][Puerto] == 0 and TR[I][6]== 0 and Rele_Out_Ant[addr][Puerto]>0:
            if DEBUG >= 1:print ('Apago rele ',addr,Puerto+1)
  ...
  ## --------- Escribir en la BD Tabla Reles el Estado RELES -------------------------
    ...
    if TP[0] == "S" and Grabar == 1:
        try:
            cursor.execute("""INSERT INTO datos (Tiempo,Ibat,Vbat,SOC,DS,Aux1,Aux2,Whp_bat,Whn_bat,Iplaca,Vplaca,Wplaca,Wh_placa,Temp,PWM) 
               VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
               (tiempo,Ibat,Vbat,SOC,DS,Aux1,Aux2,Whp_bat,Whn_bat,Iplaca,Vplaca,Wplaca,Wh_placa,Temp,PWM))
            #db.commit()
        except:
            db.rollback()
    ...
    if TP[0] == "S" and grabar_datos_s == 1:
        try:
            cursor.execute("""INSERT INTO datos_s (Tiempo,Ibat,Vbat,SOC,DS,Aux1,Aux2,Whp_bat,Whn_bat,Iplaca,Vplaca,Wplaca,Wh_placa,Temp,PWM) 
               VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
